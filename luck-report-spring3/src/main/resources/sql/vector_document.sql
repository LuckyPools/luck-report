...
COMMENT ON COLUMN luck_report_vector.vector_document.created_at IS '创建时间';

-- 普通索引
CREATE INDEX IF NOT EXISTS idx_vector_document_type
    ON luck_report_vector.vector_document (vector_type);

-- 向量相似度索引（必须建！否则搜索巨慢）
CREATE INDEX IF NOT EXISTS idx_vector_document_vector
    ON luck_report_vector.vector_document
