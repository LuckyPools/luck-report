package com.luck.agent.modules.vector.service.impl;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.luck.agent.modules.vector.domain.entity.VectorDocumentRow;
import com.luck.agent.modules.chat.service.impl.EmbeddingService;
import com.luck.agent.modules.vector.mapper.VectorDocumentMapper;
import com.luck.agent.modules.vector.service.VectorStore;
 ...
 * 使用 pgvector 扩展进行向量存储和相似度检索，支持元数据过滤
 * SQL 操作委托给 VectorDocumentMapper，由 Mapper 层通过 @DataSource("pgvector") 切换数据源
 *
 * 前置条件：
 * 1. PostgreSQL 安装 pgvector 扩展：CREATE EXTENSION IF NOT EXISTS vector;
 * 2. 创建向量文档表（见 init-schema.sql）
 *
 * 检索策略：
 * - 先通过 SQL WHERE 过滤 metadata（缩小候选集）
 * - 再使用 pgvector 的 <=> 操作符进行余弦距离排序
 * - 最后按相似度阈值过滤
 *
 * @author luck
 */
@Repository
public class PostgreSQLVectorStoreImpl implements VectorStore {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLVectorStoreImpl.class);
    ...
    public boolean deleteByMetadata(Map<String, Object> metadataFilter) {
        ...
        if (vectorType != null && !otherFilters.isEmpty()) {
            ...
            String metadataJson = gson.toJson(otherFilters);
    ...
    private VectorStoreSearchResult toSearchResult(VectorDocumentRow row) {
        ...
        Map<String, Object> metadata = gson.fromJson(row.getMetadata(), Map.class);
        if (metadata == null) {
    ...
    /**
     * 将 float[] 向量转为 pgvector 格式字符串 "[0.1,0.2,0.3]"
     *
     * @param vector 向量数组
     * @return pgvector 格式字符串
     */
