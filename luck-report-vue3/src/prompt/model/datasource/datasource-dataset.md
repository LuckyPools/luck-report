...
### (二) 创建数据源步骤
1. 所有数据源都要求用户手动添加，提示："请先手动创建数据源"。

### (三) 修改数据源步骤
1. 不允许修改数据源，提示："请手动修改数据源"。
...
### (六) 创建数据集步骤
1. 确认数据源存在。** 传入数据源名称调用【get_datasources】工具，确认目标数据源存在，有数据源才可以创建数据集，若不存在则告知用户创建数据源。
2. 生成SQL或获取Bean方法。** 根据数据源类型走不同分支：
...
3. 补充条件参数。** 若是SQL数据集且需要条件筛选功能，给数据集对象补充 parameters 字段，参数名与SQL中的占位符对应，参数类型适配需求（String/Integer/Date等）；若不同参数下SQL逻辑需动态变化，则将SQL改写为脚本式表达式（参照表达式文档）
4. 校验SQL可执行性。** 若是SQL数据集，调用【preview_data】工具验证SQL是否可执行；不可执行则调整SQL后重试一次，仍失败则终止任务并说明原因
5. 自动解析字段列表。** 若是SQL数据集，调用【build_fields】工具解析SQL生成字段列表，补充到数据集对象的 fields 字段
6. 写入数据集。** 调用【add_dataset】工具添加数据集（传入 datasourceName 和 dataset），若返回 0 则调用【restore_data】还原备份后重试1次，仍失败则终止任务并说明原因
7. 同步查询表单。** 若数据集包含 parameters 条件参数，检查报表查询表单是否已配置对应的筛选组件，缺失则调用【get_search_form】和【set_search_form】补充

### (七) 修改数据集步骤
1. 确认数据集存在。传入数据集名称调用【get_datasets】工具获取现有数据集对象，若不存在则告知用户数据集不存在
2. 修改SQL或Bean方法。基于获取的数据集对象，按用户要求修改对应字段，根据数据源类型走不同分支：
...
3. 调整条件参数。若是SQL数据集且有新增或修改条件筛选功能，调整数据集对象的 parameters 字段，参数名与SQL中的占位符对应，参数类型适配需求；若不同参数下SQL逻辑需动态变化，则将SQL改写为脚本式表达式（参照表达式文档）
4. 校验SQL可执行性。若修改了SQL，调用【preview_data】工具验证SQL是否可执行；不可执行则调整SQL后重试一次，仍失败则终止任务并说明原因
5. 重新解析字段列表。若修改了SQL，调用【build_fields】工具重新解析SQL生成字段列表，更新数据集对象的 fields 字段
6. 写入数据集。调用【update_dataset】工具更新数据集（传入 datasourceName、datasetName 和 dataset），若返回 0 则调用【restore_data】还原备份后重试1次，仍失败则终止任务并说明原因
7. 同步查询表单。若数据集的 parameters 有新增或修改，检查报表查询表单是否已配置对应的筛选组件，缺失则调用【get_search_form】和【set_search_form】补充
