...
**结构概览**：`CellDefinition` → `value(ZxingValue)` + `cellStyle(CellStyle)` + 链接/展开/条件属性等附属配置

### 1、CellDefinition 单元格定义

CellDefinition 的通用字段与普通文本单元格一致，详见[普通文本类型单元格说明](simple-text-cell.md)的 CellDefinition 部分。二维码单元格重点关注以下字段：
...
| width | Integer | 二维码宽度（px） | 最小值 `1`，默认 `100` |
| height | Integer | 二维码高度（px） | 最小值 `1`，默认 `100` |
