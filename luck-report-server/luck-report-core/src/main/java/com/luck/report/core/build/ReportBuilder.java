...
import com.luck.report.core.build.paging.BasePagination;
...
public class ReportBuilder extends BasePagination implements ApplicationContextAware {
    ...
    public void buildCell(Context context, List<Cell> cells) {
        if (cells == null) cells = context.nextUnprocessedCells();
        ...
        for (Cell cell : cells)
            try {
                ...
                Cell lastCell = cell;
                if (size == 1) lastCell = noneExpandBuilder.buildCell(dataList, cell, context);
                else if (size > 1) {
    ...
    private Map<String, Dataset> buildDatasets(ReportDefinition reportDefinition, Map<String, Object> parameters, ApplicationContext applicationContext) {
        ...
        if (datasources == null) return datasetMap;
        for (DatasourceDefinition dsDef : datasources)
            if (dsDef instanceof JdbcDatasourceDefinition) {
                ...
                try {
                    ...
                    if (ls != null) for (Dataset dataset : ls) datasetMap.put(dataset.getName(), dataset);
                } finally {
                    if (conn != null) try {
                        conn.close();
                ...
                List<Dataset> ls = ds.buildDatasets(ds.getDatasets());
                if (ls != null) for (Dataset dataset : ls) datasetMap.put(dataset.getName(), dataset);
            }
        return datasetMap;
    }
    ...
    private Row buildNewRow(Row row, Report report) {
        ...
        for (int index = 0; index < colSize; index++) {
            ...
            if (currentCell == null) throw new ReportException("Insert blank rows fail.");
            ...
            if (rowSpan > 1) currentCell.setRowSpan(rowSpan + 1);
            else {
    ...
    private Row findLastRow(Row row, Report report) {
        ...
        for (Cell cell : cells) {
            ...
            if (span == 0) span = rowSpan;
            else if (rowSpan > span) span = rowSpan;
        }
        if (span > 1) {
    ...
    private void recomputeCells(Report report, Context context) {
        ...
        for (Cell cell : lazyCells) cell.doCompute(context);
        ...
        if (pagingMode.equals(PagingMode.fitpage)) {
            ...
            for (int i = 0; i < rowSize; i++) {
                ...
                if ((i + 1) < rows.size()) {
                    ...
                    if ((rowHeight + nextRow.getRealHeight()) > height) overflow = true;
                }
                if (!overflow && row.isPageBreak()) overflow = true;
                if (overflow) {
        ...
        if (!CollectionUtils.isEmpty(summaryRows)) buildSummaryRows(summaryRows, pages);
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        ...
        for (DatasourceProvider dp : datasourceProviders) datasourceProviderMap.put(dp.getName(), dp);
