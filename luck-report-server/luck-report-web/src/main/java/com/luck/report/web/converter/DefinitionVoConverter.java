...
import com.luck.report.web.domain.vo.cell.ConditionPropertyItemVo;
import com.luck.report.web.domain.vo.cell.ConditionVo;
import com.luck.report.web.domain.vo.cell.LinkParameterVo;
...
import com.luck.report.web.domain.vo.dataset.JsonDatasetDefinitionVo;
import com.luck.report.web.domain.vo.dataset.SqlDatasetDefinitionVo;
import com.luck.report.web.domain.vo.datasource.*;
...
public final class DefinitionVoConverter {
    ...
    public static CellDefinitionVo toVo(CellDefinition cell) {
        ...
        vo.setValue(DefinitionVoConverter.toVo(cell.getValue()));
        vo.setLinkParameters(DefinitionVoConverter.toLinkParameterVoList(cell.getLinkParameters()));
        vo.setConditionPropertyItems(DefinitionVoConverter.toConditionPropertyItemVoList(cell.getConditionPropertyItems()));
        return vo;
    }
    ...
    public static Object toVo(Value value) {
        ...
        if (value instanceof SimpleValue) {
            ...
            vo.setConditions(DefinitionVoConverter.toConditionVoList(dv.getConditions()));
            return vo;
    ...
    public static List<LinkParameterVo> toLinkParameterVoList(List<LinkParameter> linkParameters) {
        if (linkParameters == null || linkParameters.isEmpty()) return null;
        List<LinkParameterVo> voList = new ArrayList<>(linkParameters.size());
        for (LinkParameter param : linkParameters) voList.add(DefinitionVoConverter.toVo(param));
        return voList;
    }
    ...
    public static LinkParameterVo toVo(LinkParameter linkParameter) {
        if (linkParameter == null) return null;
    ...
    public static List<ConditionPropertyItemVo> toConditionPropertyItemVoList(List<ConditionPropertyItem> items) {
        ...
        for (ConditionPropertyItem item : items) voList.add(DefinitionVoConverter.toVo(item));
        return voList;
    }
    ...
    public static ConditionPropertyItemVo toVo(ConditionPropertyItem item) {
        ...
        vo.setConditions(DefinitionVoConverter.toConditionVoList(item.getConditions()));
        vo.setLinkParameters(DefinitionVoConverter.toLinkParameterVoList(item.getLinkParameters()));
        return vo;
    }
    ...
    public static List<ConditionVo> toConditionVoList(List<Condition> conditions) {
        ...
        for (Condition condition : conditions) voList.add(DefinitionVoConverter.toVo(condition));
        return voList;
    }
    ...
    public static ConditionVo toVo(Condition condition) {
        ...
        if (condition instanceof BaseCondition) {
            ...
            if (operation == null || operation.isEmpty()) if (bc.getOp() != null) operation = bc.getOp().name();
            vo.setOperation(operation);
            // 将 Join 枚举转换为字符串
            if (bc.getJoin() != null) vo.setJoin(bc.getJoin().name());
    ...
    public static DatasetDefinitionVo toDatasetVo(DatasetDefinition dataset) {
        if (dataset == null) return null;
        if (dataset instanceof SqlDatasetDefinition) return DefinitionVoConverter.toVo((SqlDatasetDefinition) dataset);
        if (dataset instanceof BeanDatasetDefinition)
            return DefinitionVoConverter.toVo((BeanDatasetDefinition) dataset);
        if (dataset instanceof JsonDatasetDefinition)
            return DefinitionVoConverter.toVo((JsonDatasetDefinition) dataset);
        // 兜底：未识别子类时仅拷贝公共字段，避免静默丢字段
        DefinitionVoConverter.logger.warn("未识别的 DatasetDefinition 子类: {}", dataset.getClass().getName());
    ...
    public static List<DatasetDefinitionVo> toDatasetVoList(List<DatasetDefinition> datasets) {
        ...
        for (DatasetDefinition dataset : datasets) voList.add(DefinitionVoConverter.toDatasetVo(dataset));
        return voList;
    }
    ...
    public static DatasourceDefinitionVo toVo(DatasourceDefinition datasource) {
        ...
        if (datasource instanceof JdbcDatasourceDefinition) {
            ...
            vo = jdbcVo;
        } else if (datasource instanceof BuildinDatasourceDefinition) vo = new BuildinDatasourceDefinitionVo();
        ...
        vo.setDatasets(DefinitionVoConverter.toDatasetVoList(datasource.getDatasets()));
        return vo;
    }
    ...
    public static List<DatasourceDefinitionVo> toDatasourceVoList(List<DatasourceDefinition> datasources) {
        if (datasources == null || datasources.isEmpty()) return null;
        List<DatasourceDefinitionVo> voList = new ArrayList<>(datasources.size());
        for (DatasourceDefinition datasource : datasources) voList.add(DefinitionVoConverter.toVo(datasource));
        return voList;
    }
}
