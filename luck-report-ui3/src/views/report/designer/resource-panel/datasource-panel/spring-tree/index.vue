...
/**
 * SpringTree SpringBean 数据源树（vue3 + TS + ant-design-vue）
 *
 * 迁移说明：
 ...
 * - 数据源编辑/新建使用 SpringDialog（vue3 版本，emit save payload：{name, beanId, type, datasets, oldName}）
 * - 字段重命名使用 FieldNameDialog（vue3 版本）
 * - 字段双向数据使用 BeanMethodDialog（vue3 版本，emit save 4 个参数 name/method/clazz/oldName）
 */
...
import { buildClass } from '@/api/designer'
...
const props = withDefaults(
  ...
  { datasets: () => [], datasources: () => [], beanId: '' }
)

const emit = defineEmits<{
  (e: 'remove', name: string): void
  (e: 'update-datasource', payload: Record<string, unknown>): void
  (e: 'update-datasets', datasets: ReportDataset[]): void
}>()
...
const beanMethodDialogVisible = ref<boolean>(false)
 ...
 * 处理Bean方法保存事件
 */
function handleBeanMethodSave(name: string, method: string, clazz: string, oldName: string): void {
  const newDatasets = deepCopy(props.datasets || [])
  ...
  if (oldName && oldName !== '') {
    ...
    if (index !== -1) {
      ...
      const originalClazz = (dataset as any).clazz || ''

      dataset.name = name
      ;(dataset as any).method = method
      ;(dataset as any).clazz = clazz

      if (clazz !== originalClazz) {
        if (!clazz || clazz === '') {
          ;(dataset as any).fields = []
        } else {
          buildFieldsLocal(dataset, index, false, newDatasets)
        }
      }
  ...
  const dataset = { name, method, clazz, fields: [] } as any
  newDatasets.push(dataset)
  const index = newDatasets.length - 1
  datasetExpanded[index] = true
...
function refreshDatasetAction(dataset: ReportDataset, index: number): void {
  const newDatasets = deepCopy(props.datasets || [])
  const targetDataset = newDatasets.find((d) => d.name === dataset.name)
  buildFieldsLocal(targetDataset as any, index, true, newDatasets)
}
...
function deleteFieldAction(dataset: ReportDataset, field: ReportDatasetField, fieldIndex: number): void {
  showConfirm(`${t('tree.delFieldConfirm')}[${field.name}]?`).then(() => {
    const newDatasets = deepCopy(props.datasets || [])
    const targetDataset = newDatasets.find((d) => d.name === dataset.name)
    if (targetDataset && (targetDataset as any).fields) {
      ;(targetDataset as any).fields.splice(fieldIndex, 1)
      emit('update-datasets', newDatasets)
    }
  })
}
 ...
 * 构建字段列表
 */
async function buildFieldsLocal(dataset: ReportDataset, _index?: number, refresh: boolean = false, newDatasets: ReportDataset[] | null = null): Promise<void> {
  const defaultFields = (dataset as any).fields
  ...
  try {
    const fields = await buildClass((dataset as any).clazz)
    ;(dataset as any).fields = fields
    if (newDatasets) {
      emit('update-datasets', newDatasets)
    }
  } catch (error: any) {
 ...
 * 处理Spring数据源保存事件
 */
function handleSpringDatasourceSave(datasourceData: any): void {
