...
} from '@/api/bussiness-knowledge/idnex'
import {
...
const businessKnowledgeList = ref<Idnex[]>([])
 ...
 * 编辑业务知识
 */
const editKnowledge = (knowledge: Idnex) => {
  isEdit.value = true
 ...
 * 删除业务知识
 */
const deleteKnowledge = async (knowledge: Idnex) => {
 ...
 * 切换生效状态
 */
const toggleEnabled = async (knowledge: Idnex, enabled: boolean) => {
  if (!knowledge.id) return
 ...
 * 重试向量化
 */
const retryEmbedding = async (knowledge: Idnex) => {
  if (!knowledge.id) return
