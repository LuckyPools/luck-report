 ...
 *
 * 设计说明：
 * - 使用 `as const` 让 TS 推断出 readonly 的具体字面量类型
 * - 键结构与 zh.ts 保持一致，便于类型校验翻译覆盖度
 * - 字段差异（如 undo.undo 在英文中省略）保留原文以避免误改
 */
const en = {
    placeholder: {
        currentStage: 'Current stage: vue3 skeleton ready, awaiting views migration',
        piniaReady: 'Pinia is ready',
        piniaFailed: 'Pinia check failed'
    },
