 ...
 *
 * 设计说明：
 * - 使用 `as const` 让 TS 推断出 readonly 的具体字面量类型（保护键值不被改写）
 * - 顶层 const 命名为 zh，类型通过 typeof 推导
 * - export default 沿用原 JS 写法，避免 locales/index.ts 改动
 */
const zh = {
    placeholder: {
        currentStage: '当前阶段：vue3 骨架已就绪，等待 views 迁移',
        piniaReady: 'Pinia 已注入',
        piniaFailed: 'Pinia 检查失败'
    },
