...
import { previewData, buildFields } from '@/utils/tools'
import {DatasetSchema, getSqlDatasetTemplate, normalizeDataset, validateDataset} from './schema'
...
export const getDatasetsTool: ToolDefinition<{
  ...
  execute: async ({ datasourceName, datasetName }) => {
    return getDatasets({ datasourceName, datasetName })
  },
  readOnly: true,
...
【数据规范】dataset 必须包含 name/sql/fields，fields 必须通过 build_fields 工具生成。。`,
  ...
  execute: async ({ datasourceName, dataset }) => {
    ...
    return addDataset({ datasourceName, dataset: normalized })
  },
  readOnly: false,
  requireConfirm: false,
...
【数据规范】dataset 必须包含 name/sql/fields，fields 必须通过 build_fields 工具生成。`,
  ...
  execute: async ({ datasourceName, datasetName, dataset }) => {
    ...
    const normalized = normalizeDataset(dataset)
    return updateDataset({ datasourceName, datasetName, dataset: normalized })
  },
  readOnly: false,
  requireConfirm: false,
  validate: ({ dataset }) => validateDataset(dataset)
...
export const removeDatasetTool: ToolDefinition<{
  ...
  execute: async ({ datasourceName, datasetName }) => {
    return removeDataset({ datasourceName, datasetName })
  },
  readOnly: false,
  requireConfirm: true
}
...
export const previewDataTool: ToolDefinition<{
  ...
  execute: async ({ sql, type, parameters, username, password, driver, url, name }) => {
    return previewData({ sql, type, parameters, username, password, driver, url, name })
  },
  readOnly: true,
  requireConfirm: false
}
...
export const buildFieldsTool: ToolDefinition<{
  ...
  execute: async ({ sql, type, parameters, username, password, driver, url, name }) => {
    return buildFields({ sql, type, parameters, username, password, driver, url, name })
  },
  readOnly: true,
  requireConfirm: false
}
    ...
    if (structureErrors.length === 0 && normalized.sql) {
      try {
        preview = await previewData({
          ...
          type: 'buildin',
          name: datasourceName,
          parameters: Array.isArray(normalized.parameters) && normalized.parameters.length > 0 ? normalized.parameters : undefined
        })
      } catch (e: any) {
