/**
 * 加载文档节点（公共）
 * 报表主工作流入口节点，调 load_report_introduce 加载 intent.requiredDocs
 * 行为：
...
export function buildLoadDocsNode() {
  return withInput(async (state: ReportState, _config, runtime) => {
    ...
    if (missingDocs.length === 0) {
      const skipCallId = buildToolCallId(stepId, 'load_report_introduce_skip', runtime.runId)
      runtime.emitEvent({ mode: 'updates', event: { nodeId: stepId, output: { type: 'tool_call', toolCallId: skipCallId, toolName: 'load_report_introduce', input: { fileNames: docs } }, status: 'running' }, timestamp: Date.now() })
      runtime.emitEvent({ mode: 'updates', event: { nodeId: stepId, output: { type: 'tool_result', toolCallId: skipCallId, toolName: 'load_report_introduce', result: '已命中跨 turn 文档缓存，本次跳过加载' }, status: 'success' }, timestamp: Date.now() })
      return { searchResults: { docRefs: docs } } as ReportStateUpdate
    }

    // 缺失加载：调工具 → 写缓存 → 写 messages
    const toolCallId = buildToolCallId(stepId, 'load_report_introduce', runtime.runId)
    runtime.emitEvent({ mode: 'updates', event: { nodeId: stepId, output: { type: 'tool_call', toolCallId, toolName: 'load_report_introduce', input: { fileNames: missingDocs } }, status: 'running' }, timestamp: Date.now() })
    const result = await runtime.toolRegistry.executeTool('load_report_introduce', { fileNames: missingDocs })
    runtime.emitEvent({ mode: 'updates', event: { nodeId: stepId, output: { type: 'tool_result', toolCallId, toolName: 'load_report_introduce', result: formatDocsAsText(result) }, status: result ? 'success' : 'failed' }, timestamp: Date.now() })
