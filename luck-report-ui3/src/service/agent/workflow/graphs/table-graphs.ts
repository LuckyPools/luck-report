...
export function modifyCellGraph(): CompiledReportGraph {
    ...
    const planCellStructure = createLLMDecideNode({
        ...
        description:
            ...
            '}\n\n' +
            '【关键规则】\n' +
            '1、每行是一个batch，不要把多行合并为一个batch\n' +
            '2、禁止捏造数据，dataset 类型单元格的 datasetName 和 property 必须从 [可用数据集清单] 中选取\n' +
            '3、若 [可用数据集清单] 缺失或字段未注入，必须先调 get_datasets 工具拿到真实 datasetName/fields 后再规划\n' +
            '4、合并单元格只规划为1个cell，不要拆成多列。例如cellsData中A1是colSpan=4的合并单元格时，该行只规划1个cell{col:1}，不要拆成col=1/2/3/4四个cell'
    })
    ...
    const writeCellsBatch = createLLMDecideNode({
        ...
        description: (state: ReportState) => {
            ...
            // 关键决策点：动态判断本批次是否含 dataset 类型，仅在此情况下强制 LLM 先调 get_datasets 校验
            // 简单单元格（simple/expression）场景不强制，避免无意义的 token 消耗
            const hasDatasetCell = batch.cells.some(c => c.valueType === 'dataset')
            if (hasDatasetCell) {
                desc +=
                    '\n【强约束 — 防捏造】本批次含 dataset 类型单元格：\n' +
                    '1、若上文 [可用数据集清单] 已列出本批引用的 datasetName 且包含目标 property，可直接使用；\n' +
                    '2、若 [可用数据集清单] 缺失、字段不全、或本批引用的 datasetName/字段不在清单中，**必须**先调 get_datasets 工具拿到真实 datasetName/fields 列表后再 write_cells；\n' +
                    '3、禁止使用 cellBatchPlan/规划阶段推测的 datasetName 或 property，禁止凭空编造字段名（如 id/name/email 等）。\n'
            }
            desc +=
                '\n只写当前行的单元格，不要写其他行。' +
